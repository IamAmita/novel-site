# 小説ステータステーブル（novel_statuses）

---

## 概要
小説（novels）の公開状態や管理状態を表すテーブル。各小説の状態履歴や現在の状態を管理し、システム全体で一貫したステータス管理を実現する。
...
| UNIQUE                 | 複合一意 | 小説ID・ステータスマスタIDの組み合わせ一意 | novel_id, status_id |
...
- **履歴管理**: 小説ごとに複数の状態履歴を持つことができる。
- **拡張性**: ステータス種別や値の追加・変更が容易。
- **一意性**: 小説ごとに同じステータスマスタIDの重複登録は不可。
...
1. **データ整合性**: 小説の状態は必ず本テーブルのIDを参照
2. **拡張性**: ステータス追加時は値の一意性・表示順に注意
