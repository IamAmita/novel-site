...
## 概要
ユーザーが投稿したレビューを管理するテーブルです。多様な対象（小説・設定等）にレビューでき、評価点とレビュー本文を含みます。レビュー対象はclass_id（master_classes.id, 2桁:10〜99）とtable_id（master_tables.id, 4桁:1000〜9999）で管理します。
...
| table_id| int        | ○    |      | レビュー対象テーブルID（master_tables.id, 4桁:1000〜9999） |
...
| UNIQUE | 複合一意 | 重複レビュー防止 | user_id, class_id, table_id, target_id |
...
### ユニーク制約
- `user_id, class_id, table_id, target_id`: 1ユーザーにつき1対象に1つのレビューのみ許可
...
### レビュー対象（class_id, table_id, target_id）
