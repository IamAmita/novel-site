# notifications テーブル詳細

---

## 概要
ユーザーへの通知を管理するテーブルです。通知種別、通知内容、既読状態、優先度などを含みます。

---

## テーブル定義

| 属性名             | 型         | 必須 | 一意 | 説明                                 |
|--------------------|------------|------|------|--------------------------------------|
| id                 | int        | ○    | ○    | 通知ID（主キー）                     |
| user_id            | int        | ○    |      | 通知対象ユーザーID                   |
| type               | tinyint    | ○    |      | 通知種別                             |
| priority           | tinyint    | ○    |      | 優先度                               |
| target_type        | tinyint    |      |      | 通知対象の種別                       |
| target_id          | int        |      |      | 通知対象のID                         |
| related_user_id    | int        |      |      | 関連ユーザーID（コメントした人など） |
| is_read            | tinyint    | ○    |      | 既読/未読フラグ                      |
| is_deleted         | tinyint    | ○    |      | 論理削除フラグ                       |
| deleted_at         | datetime   |      |      | 削除日時                             |
| deleted_by         | int        |      |      | 削除者ID                             |
| created_at         | datetime   | ○    |      | 通知日時                             |

---

## インデックス

| インデックス名 | 種類 | 説明 | カラム |
|----------------|------|------|--------|
| PRIMARY KEY | 主キー | 通知IDの主キー | id |
| INDEX | 通常 | 通知対象ユーザー検索用 | user_id |
| INDEX | 通常 | 通知種別検索用 | type |
| INDEX | 通常 | 優先度検索用 | priority |
| INDEX | 通常 | 既読状態検索用 | is_read |
| INDEX | 通常 | 通知日時検索用 | created_at |
| INDEX | 複合 | 通知対象検索用 | target_type, target_id |
| INDEX | 複合 | ユーザー・削除状態検索用 | user_id, is_deleted |
| INDEX | 通常 | 削除日時検索用 | deleted_at |

---

## 制約条件

### 主キー制約
- `id`: 自動採番（AUTO_INCREMENT）

### 外部キー制約
- `user_id` → `users.id`: ユーザーテーブルを参照
- `target_type` → `target_types.id`: 対象種別定義テーブルを参照
- `related_user_id` → `users.id`: 関連ユーザーテーブルを参照
- `deleted_by` → `users.id`: 削除者テーブルを参照

### チェック制約
- `priority`: 0以上2以下の値のみ許可
- `is_read`: 0または1の値のみ許可
- `is_deleted`: 0または1の値のみ許可
- `type`: 0～6のいずれかである必要があります
- `target_type`: 0、1、2、3、4、5、6のいずれかである必要があります

---

## 設計補足

### 通知種別（type）
- `0`: コメント通知
- `1`: いいね通知
- `2`: フォロー通知
- `3`: レビュー通知
- `4`: システム通知
- `5`: セキュリティ通知
- `6`: 作品通知

### 優先度（priority）
- `0`: 通常（デフォルト）
- `1`: 重要（フォロー、レビューなど）
- `2`: 緊急（システムメンテナンスなど）

### 通知対象（target_type, target_id）
- `0`: コメントへの通知
- `1`: 小説作品への通知
- `2`: 設定（世界観・キャラ等）への通知
- `3`: 話（エピソード）への通知
- `4`: レビューへの通知
- `5`: ユーザーへの通知
- `6`: グループへの通知

### 関連ユーザー（related_user_id）
- 通知を発生させたユーザー
- 例：コメント通知の場合、コメントしたユーザー

### 既読/未読フラグ（is_read）
- `0`: 未読
- `1`: 既読
- ユーザーが通知を開いた時に更新

### 論理削除フラグ（is_deleted）
- `0`: 削除されていない（通常状態）
- `1`: 論理削除済み
- ユーザーが通知を「削除」した場合に設定
- 削除された通知は一覧に表示しない

### 削除日時（deleted_at）
- 論理削除が実行された日時
- 削除履歴の記録用

### 削除者ID（deleted_by）
- 論理削除を実行したユーザーID
- 通常は通知対象ユーザー自身
- 削除履歴の記録用

---

## 関連テーブル

- `users`: 多対1の関係（通知対象ユーザー）
- `users`: 多対1の関係（関連ユーザー）
- `target_types`: 多対1の関係（対象種別定義）
- `templates`: 1対多の関係（通知種別からテンプレートを特定）

---

## 運用上の注意点

1. **通知の重複**: 同じ内容の通知が重複しないよう制御
2. **既読管理**: ユーザーが通知を開いた時に既読フラグを更新
3. **論理削除**: ユーザーによる削除操作は論理削除で対応
4. **削除履歴**: 削除日時と削除者を記録して監査対応
5. **パフォーマンス**: 大量の通知データの効率的な管理
6. **表示制御**: 論理削除された通知は一覧に表示しない
7. **テンプレート連携**: 通知種別（type）からtemplatesテーブルのテンプレートを特定
8. **多言語対応**: ユーザーの言語設定に応じて適切なテンプレートを選択