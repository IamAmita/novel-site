...
## 概要
ユーザーへの通知を管理するテーブルです。通知内容や対象情報を持ち、**既読・未読（is_read）と削除フラグ（delete_flg）は本テーブルで管理**します。通知の重要・緊急など複合的な状態はnotification_statusesテーブル（master_statuses参照）で管理します。通知種別（notification_type）はmaster_actionsテーブルで一元管理します。
...
| delete_flg         | tinyint    | ○    |      | 削除フラグ（0:有効, 1:削除）         |
...
- `delete_flg`: 0または1の値のみ許可（0:有効, 1:削除）
...
## 設計補足
- 既読・未読（is_read）と削除フラグ（delete_flg）は本テーブルで管理します。
...
- 通知本体には状態情報（is_read, delete_flg）のみ持たせ、複合状態は中間テーブルで拡張可能です。
...
2. **既読・未読・削除管理**: is_read, delete_flgフラグで管理（0:未読/有効, 1:既読/削除）
