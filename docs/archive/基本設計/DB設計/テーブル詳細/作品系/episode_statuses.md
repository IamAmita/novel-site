# 話ステータステーブル（episode_statuses）

---

## 概要
話（episodes）の公開状態や管理状態を表すステータス履歴テーブル。各話の状態履歴や現在の状態を管理し、システム全体で一貫したステータス管理を実現する。
...
| episode_id    | int           | ○    |      | 話ID（episodes.idを参照、NOT NULL） |
...
| UNIQUE                 | 複合一意 | 話ID・ステータスマスタIDの組み合わせ一意 | episode_id, status_id |
...
### 外部キー制約
- `episode_id` → `episodes.id`（NOT NULL）
...
### 一意制約
- `episode_id`, `status_id`の組み合わせは一意
...
- **履歴管理**: 話ごとに複数の状態履歴を持つことができる。
- **拡張性**: ステータス種別や値の追加・変更が容易。
- **一意性**: 話ごとに同じステータスマスタIDの重複登録は不可。
