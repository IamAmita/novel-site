...
- 初期セット（2026-08-22決定）: **スパム／著作権侵害／不適切な内容／嫌がらせ／その他**の5項目。他のマスタ（SettingTemplate等）と同様、自由に追加・編集・削除できる
...
- `User.status`の自動復帰の仕組み（2026-08-22決定）: **アクセス時に都度チェック**する方式。ログイン・リクエスト処理のタイミングで`suspend`の`expires_at`を過ぎていないか確認し、過ぎていればその場で`User.status`を`active`に戻す。定期バッチは用意しない
