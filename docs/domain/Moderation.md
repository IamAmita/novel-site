...
- 通報対象が`pen_name`でWorldを保持している場合（2026-08-22決定）: 「コンテンツの削除」操作は選択できない（削除ボタンを無効化する等）。かわりに対象ユーザーへの警告／一時停止／永久停止で対応する
...
- PenNameの削除制約（2026-08-22決定）: 管理者による強制削除であっても、[User.md](User.md)の本人操作と同じルール（**Worldを1件も保持していなければ削除可能、1件でも保持していれば削除不可**）を適用する。例外は設けない。Worldを保持しているPenNameが通報された場合、管理者はPenName単位では削除せず、そのPenNameが属するUserを`suspend`／`ban`することで対応する（既存の「アカウント停止時、所有する全Novelを自動的に`private`化する」仕組みで実質的な対処が完結する）
